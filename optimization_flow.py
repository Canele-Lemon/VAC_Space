수정해야할 부분
1. start_vac_optimization 한번 한 뒤 또 start_vac_optimization하면 그래프 초기화 안되어 있음
2. vac_label_pixmap_* 쪽에 적절하지 않은 픽스맵 올 때가 있는데 꼬여있는거 같음. 수정 부탁
...
현재까지 코드(일부분):
    ...
    def start_vac_optimization(self):
        ...
        if not self._confirm_vac_optimization_target():
            logging.info("[VAC Optimization] User canceled optimization target confirmation.")
            return
