    ...
    def _apply_vac_from_db_and_measure_on(self):
        ...
        def _after_read(vac_dict):
            ...
            def _after_on(store_on):
                ...
                self.label_processing_step_5, self.movie_processing_step_5 = self.start_loading_animation(self.ui.vac_label_pixmap_step_5, 'processing.gif')
        ...
        self._write_vac_to_tv(vac_data_by_predict, on_finished=_after_write)
    ...
    def _write_vac_to_tv(self, vac_data, on_finished):
        self.label_processing_step_3, self.movie_processing_step_3 = self.start_loading_animation(self.ui.vac_label_pixmap_step_3, 'processing.gif')
    ...
    def start_VAC_optimization(self):
        ...
        process_complete_icon_path = cf.get_normalized_path(__file__, '..', '..', 'resources/images/pictures', 'process_complete.png')
...
지금까지 작성된 코드인데 프로세스가 순서가 꼬이는 부분이 있어 다음과 같은 순서로 프로세스가 흘러가고 gif가 작동할 수 있도록 검토 및 수정 요청드립니다.

1. vac off 전환 및 측정 - self.label_processing_step_1, self.movie_processing_step_1 = self.start_loading_animation(self.ui.vac_label_pixmap_step_1, 'processing.gif') 시작
2. lut db fetch 후 예측 및 보정 - self.label_processing_step_2, self.movie_processing_step_2 = self.start_loading_animation(self.ui.vac_label_pixmap_step_2, 'processing.gif') 시작
3. vac on 전환 후 보정 lut 적용 - self.label_processing_step_3, self.movie_processing_step_3 = self.start_loading_animation(self.ui.vac_label_pixmap_step_3, 'processing.gif') 시작
4. 보정 lut 적용 기준 측정 - self.label_processing_step_4, self.movie_processing_step_4 = self.start_loading_animation(self.ui.vac_label_pixmap_step_4, 'processing.gif') 시작
5. 결과 평가 - self.label_processing_step_5, self.movie_processing_step_5 = self.start_loading_animation(self.ui.vac_label_pixmap_step_5, 'processing.gif') 시작

만약 결과 평가 fail 시 다시 2번에서 '보정'부터 시작해서 2,3,4,5 반복.

gif 끝내는건 
self.stop_loading_animation(self.label_processing_step_{}, self.movie_processing_step_{}) 
self.ui.vac_label_pixmap_step_{}.setPixmap(self.process_complete_pixmap)
으로 합니다.
