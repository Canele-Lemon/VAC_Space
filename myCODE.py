    ...
    def _session_step(self):
        ...
        if s['phase'] == 'gamma':
            if s['p_idx'] >= len(s['patterns']):
                s['phase'] = 'colorshift'
                s['cs_idx'] = 0
                QTimer.singleShot(60, lambda: self._session_step(self))
                return

            if s['g_idx'] >= len(s['gray_levels']):
                s['g_idx'] = 0
                s['p_idx'] += 1
                QTimer.singleShot(40, lambda: self._session_step(self))
                return
            ...
            QTimer.singleShot(delay, lambda p=pattern, g=gray: self._trigger_gamma_pair(self, p, g))

        elif s['phase'] == 'colorshift':
            if s['cs_idx'] >= len(s['cs_patterns']):
                s['phase'] = 'done'
                QTimer.singleShot(0, lambda: self._session_step(self))
                return

            pname, r, g, b = s['cs_patterns'][s['cs_idx']]
            self.changeColor(f"{r},{g},{b}")
            QTimer.singleShot(s['cs_settle_ms'], lambda pn=pname: self._trigger_colorshift_pair(self, pn))
    ...
    def _trigger_gamma_pair(self, pattern, gray):
        ...
        def handle(role, res):
            ...
            if 'main' in s['_gamma'] and 'sub' in s['_gamma']:
                ...
                s['g_idx'] += 1
                QTimer.singleShot(30, lambda: self._session_step(self))
    ...
    def _trigger_colorshift_pair(self, patch_name):
        ...
        def handle(role, res):
            ...
            if 'main' in s['_cs'] and 'sub' in s['_cs']:
                ...
                s['cs_idx'] += 1
                QTimer.singleShot(80, lambda: self._session_step(self))
