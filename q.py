# -*- coding: utf-8 -*-
# Widget_vacspace 내부에 그대로 넣어 쓰세요.
# 필요 유틸/헬퍼는 클래스 바깥의 모듈로 빼셔도 OK.
...
def _set_item(table, row, col, value):
    _ensure_row_count(table, row)
...
def _session_step(self):
    ...
    if s['phase'] == 'gamma':
        if s['p_idx'] >= len(s['patterns']):
            s['phase'] = 'colorshift'
            s['cs_idx'] = 0
            QTimer.singleShot(60, lambda: _session_step(self))
            return

        if s['g_idx'] >= len(s['gray_levels']):
            s['g_idx'] = 0
            s['p_idx'] += 1
            QTimer.singleShot(40, lambda: _session_step(self))
            return
        ...
        QTimer.singleShot(delay, lambda p=pattern, g=gray: _trigger_gamma_pair(self, p, g))

    elif s['phase'] == 'colorshift':
        if s['cs_idx'] >= len(s['cs_patterns']):
            s['phase'] = 'done'
            QTimer.singleShot(0, lambda: _session_step(self))
            return

        pname, r, g, b = s['cs_patterns'][s['cs_idx']]
        self.changeColor(f"{r},{g},{b}")
        QTimer.singleShot(s['cs_settle_ms'], lambda pn=pname: _trigger_colorshift_pair(self, pn))
...
def _trigger_gamma_pair(self, pattern, gray):
    ...
    def handle(role, res):
        ...
        if 'main' in s['_gamma'] and 'sub' in s['_gamma']:
            _consume_gamma_pair(self, pattern, gray, s['_gamma'])
            s['g_idx'] += 1
            QTimer.singleShot(30, lambda: _session_step(self))
...
def _consume_gamma_pair(self, pattern, gray, results):
    ...
    if pattern == 'white':
        ...
        table = self.ui.table_measure_results_main_2
        cols = profile.table_cols
        _set_item(table, gray, cols['lv'], f"{lv_m:.6f}")
        _set_item(table, gray, cols['cx'], f"{cx_m:.6f}")
        _set_item(table, gray, cols['cy'], f"{cy_m:.6f}")
        ...
        if profile.ref_store is not None:
            ...
            if ref is not None and 'd_cx' in cols and 'd_cy' in cols:
                lv_r, cx_r, cy_r = ref
                _set_item(table, gray, cols['d_cx'], f"{(cx_m - cx_r):.6f}")
                _set_item(table, gray, cols['d_cy'], f"{(cy_m - cy_r):.6f}")
...
def _trigger_colorshift_pair(self, patch_name):
    ...
    def handle(role, res):
        ...
        if 'main' in s['_cs'] and 'sub' in s['_cs']:
            _consume_colorshift_pair(self, patch_name, s['_cs'])
            s['cs_idx'] += 1
            QTimer.singleShot(80, lambda: _session_step(self))
...
def _finalize_session(self):
    ...
    gamma_vec = _compute_gamma_series(lv_series)
    for g in range(256):
        if np.isfinite(gamma_vec[g]):
            _set_item(table, g, cols['gamma'], f"{gamma_vec[g]:.6f}")
    ...
    if profile.ref_store is not None and 'd_gamma' in cols:
        ...
        ref_gamma = _compute_gamma_series(ref_lv)
        dG = gamma_vec - ref_gamma
        for g in range(256):
            if np.isfinite(dG[g]):
                _set_item(table, g, cols['d_gamma'], f"{dG[g]:.6f}")
...
def _apply_vac_from_db_and_measure_on(self):
    ...
    start_viewing_angle_session(
        self, profile=profile_corr, gamma_lines=gamma_lines_corr,
        gray_levels=list(range(256)), patterns=('white','red','green','blue'),
        colorshift_patterns=op.colorshift_patterns,
        first_gray_delay_ms=3000, cs_settle_ms=1000,
        on_done=_after_corr
    ...
    G_ref = _compute_gamma_series(lv_ref)
    G_on  = _compute_gamma_series(lv_on)
...
def _build_A_from_artifacts(self, artifacts, comp):
    knots = np.asarray(artifacts["knots"], dtype=np.int32)
    K = len(knots)
    coef = np.asarray(artifacts["components"][comp]["coef"], dtype=np.float32)
    s = artifacts["components"][comp]["feature_slices"]
    s_high_R = slice(s["high_R"][0], s["high_R"][1])
    s_high_G = slice(s["high_G"][0], s["high_G"][1])
    s_high_B = slice(s["high_B"][0], s["high_B"][1])
    beta_R = coef[s_high_R]; beta_G = coef[s_high_G]; beta_B = coef[s_high_B]
    Phi = self._stack_basis(knots)  # (256,K)
...
# ========= TV/DB I/O (당신 프로젝트의 기존 함수/스레드에 연결만 바꾸세요) =========
