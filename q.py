...
이제 이렇게 로드할 모델들을 기반으로 예측과 보정을 하기 위해 자코비안 npy 파일, *_model_pkl 파일을 만들어야 돼요.

우선 자코비안 npy 파일부터요.
아래는 estimate_jacobian.py 라는 자코비안 J_g 계산 코드인데 제대로된 계산을 위해 데이터셋 확인을 하는 것부터 시작할게요. Base PK(ref)=3008, target PK=3157로 해볼게요.
